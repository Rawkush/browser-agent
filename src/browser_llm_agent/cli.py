...
Advanced debugging — when the basic 5 steps stall or the bug is non-obvious:
  ...
  INSTRUMENT (when you can't see logs)
    ...
    do NOT rely on console.log. Instead, inject diagnostic data into the response
    itself — add state to error messages, emit debug SSE events, write to a temp
    file, or return extra fields in the JSON response. Remove after diagnosis.
